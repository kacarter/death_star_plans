...
def background_animation():
    ...
    for b in range(background_pixels):
        if b % 6 in [background_counter, (background_counter + 1) % 6, (background_counter + 2) % 6]:
            pixels[b + (num_spokes * strip_length)] = (0, 0, 255)
...
def play_animation():
        ...
        for j in range(num_spokes):
            for k in range(spoke_length):
                if i >= 10 and k >= 10:
                    ...
                    if k % 6 in [i % 6, (i + 1) % 6, (i + 2) % 6]:
                        pixels[(j * strip_length) + k] = (164 + extra_brightness, extra_brightness * 1.5, extra_brightness * 1.5)
                else:
                    pixels[(j * strip_length) + k] = (0, 150, 0)
                    if k % 6 in [i % 6, (i + 1) % 6, (i + 2) % 6]:
                        pixels[(j * strip_length) + k] = (150, 0, 0)
    ...
    for i in range(strip_length - spoke_length):
        pixels[i + ((num_spokes - 1) * strip_length) + spoke_length] = random.choice(colors)
            ...
            for j in range(num_spokes):
                for k in range(spoke_length):
                    if (k >= 10):
                        ...
                        pixels[(j * strip_length) + k] = (extra_brightness * 1.5, 164 + extra_brightness, extra_brightness * 1.5)
                        if k % 6 in [i % 6, (i + 1) % 6, (i + 2) % 6]:
                            pixels[(j * strip_length) + k] = (
                            164 + extra_brightness, extra_brightness * 1.5, extra_brightness * 1.5)
                    else:
                        pixels[(j * strip_length) + k] = (0, 150, 0)
                        if k % 6 in [i % 6, (i + 1) % 6, (i + 2) % 6]:
                            pixels[(j * strip_length) + k] = (150, 0, 0)
